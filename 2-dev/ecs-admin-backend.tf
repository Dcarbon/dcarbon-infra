locals {
  backend-admin-ecr-image-tag = "latest"
  backend-admin-task-definition-container-ports = {
    default : 80
  }
  backend-admin = {
    task-definition-container-name : "${var.PROJECT_NAME}-${var.ENV}-${var.PROJECT_SERVICES.BACKEND_ADMIN}-container",
